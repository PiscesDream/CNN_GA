...
datasets = cnn.load_data('../../../Data/mnist/mnist.pkl.gz', 500)
x, y = cnn.load_data('../../../Data/mnist/mnist.pkl.gz', 5000)[0]
y = list(y.eval())
x = x.eval()
l = int(len(y)*0.7)
...
def train_and_get(c, lr):
    ...
    c.fit_lr(n_epochs = 100, slient = True)
...
def cnn_fitness(base):
    ...
    c.set_datasets(datasets, batch_size = 50) 
...
def cnn_gen_init():
    ...
    return rng.randint(1, 11, size=(4,))
...
def cnn_plot_f(base, clr='none', clean=False):
    print [(base[0], (base[1], base[1]), (2, 2)), (base[2], (base[3], base[3]), (2, 2))]
...
if __name__ == '__main__':
    ...
    MyGA = ga5.GA(cnn_fitness, {'iter_maximum':200}, 
                    10, cnn_gen_init,
                    0.75, cnn_crossover, 0.15, cnn_mutation, 
