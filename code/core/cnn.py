...
class CNN(object):
    ...
    def set_datasets(self, datasets, batch_size=500):
    ...
    def fit(self, n_epochs = 200, learning_rate=0.01,
            test_model_on = True, slient = False):
        index = self.index

        if not slient: print 'building training model...'
...
#            start_time = time.time()
            ...
            for minibatch_index in xrange(self.n_train_batches):
                ans = train_model(minibatch_index)
                if not slient: print '\tL of (%03d/%03d) = %f\r'%(minibatch_index, self.n_train_batches, ans),
#                print '\tL of (%03d/%03d) = %f'%(minibatch_index, n_train_batches, train_model(minibatch_index))
            if not slient: print ''
            if not slient and test_model_on: print 'error = %f' % test_model()
    ...
    def reset_weight(self):
        for cnn in self.cnn_layers:
            ...
            w_value = numpy.asarray(
                            ...
                            dtype = theano.config.floatX) 
            cnn.W.set_value(w_value)

            b_value = numpy.zeros((filter_shape[0],), dtype = theano.config.floatX)
            cnn.b.set_value(b_value)


    def compile_lr(self, learning_rate=0.01):
#        print 'building training model...'
        index = self.index
        self.train_model_lr = theano.function([index], self.loss, 
            updates = self.updates,
            givens = {
                self.x: self.train_set_x[index * self.batch_size: (index + 1) * self.batch_size],
                self.y: self.train_set_y[index * self.batch_size: (index + 1) * self.batch_size],
    ...
    def fit_lr(self, n_epochs = 200, slient = False):
        index = self.index

#        print 'training with lr: %lf...' % learning_rate
        maxiter = n_epochs
        iteration = 0
        while iteration < maxiter:
            iteration += 1
            if not slient: print 'iteration %d' % iteration
            for minibatch_index in xrange(self.n_train_batches):
                ans = self.train_model_lr(minibatch_index)
                if not slient: print '\tL of (%03d/%03d) = %f\r'%\
                        (minibatch_index, self.n_train_batches, ans),
            if not slient: print ''
