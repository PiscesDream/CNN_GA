# lab7
# copy from lab5
# while training, the cnn will not see the test set
...
def get_dataset(all_x, all_y, alpha=0.4, beta=0.7, num = None):
...
#    all_y = all_y[random_index]
    ...
    sep1 = int(total_size * alpha)
    sep2 = int(total_size * beta)

    train_set = (all_x[:sep1], all_y[:sep1])
    valid_set = (all_x[sep1:sep2], all_y[sep1:sep2])
    test_set = (all_x[sep2:], all_y[sep2:])
...
def continue_train(dataset, filename='model.dat'):
    k = raw_load(filename)
    print k
    k.fit(dataset, n_epochs=1000, learning_rate=0.0001)
    raw_dump(k, 'model.dat') 
    return k

def train_new(dataset):
    k = cnn(dim_in = 1, size_in = (A, A), size_out = N, 
            nkerns = [(10, (5, 5), (2, 2)), (10, (5, 5), (2, 2))], 
            nhiddens=[300])
